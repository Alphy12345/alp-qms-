...
const InspectionPlan = () => {
  ...
  useEffect(() => {
    if (!partId) return;
    let cancelled = false;
    fetch(`http://localhost:8000/api/v1/parts/${partId}/with-location`)
  ...
  useEffect(() => {
    ...
    fetch(`http://localhost:8000/api/v1/parts/${partId}`)
      .then((res) => (res.ok ? res.json() : Promise.reject(new Error(res.statusText))))
      .then((part) => {
      ...
      if (!documentId && partData.pdfUrl && partId) {
        ...
        if (urlMatch) {
          ...
          try {
            ...
            const documentsResponse = await fetch(`http://localhost:8000/api/v1/documents?part_id=${partId}`);
...
const loadNotes = async () => {
  ...
  const handleConfirmInspectionStatus = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/v1/parts/${partId}/toggle-inspection-plan`, {
        method: 'PATCH',
